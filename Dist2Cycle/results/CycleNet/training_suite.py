...
from src.model.Dist2CycleRegressor_cycle import Dist2CycleRegressor
...
save_appendix = '_' + time.strftime("%Y%m%d%H%M%S")
res_dir = os.path.join(file_dir, 'results/{}'.format(save_appendix))
print('Results will be saved in ' + res_dir)
if not os.path.exists(res_dir):
    os.makedirs(res_dir)
copy('training_suite.py', res_dir)
# save command line input
cmd_input = 'python ' + ' '.join(sys.argv) + '\n'
with open(os.path.join(res_dir, 'results.txt'), 'a') as f:
    f.write(cmd_input)
...
if __name__=="__main__":
    ...
    parser.add_argument('--infeaturescols', default=10,
    ...
    if not use_hodge:
        ...
        model_params['in_feats']=2*infeaturescols+3
    ...
    model=Dist2CycleRegressor(in_feats=model_params['in_feats'],
                         ...
                         verbose=verbose, use_hodge = use_hodge)
    ...
    for epoch in range(epochs):
        ...
        for batch_id, batch_data in enumerate(train_loader):
            ...
            if not use_hodge:
                ...
                batched_graph = dgl.unbatch(graph)
                eigenval = torch.cat([bg.edata['S'][:bg.ndata['V'].size(0),0:infeaturescols] for bg in batched_graph], dim = 0)
                eigenvecs = torch.cat((graph.ndata['V'][:,0:infeaturescols], eigenval), dim = 1)
                #eigenvecs=torch.cat((graph.ndata['V'][:,0:infeaturescols], graph.edata['S'][:graph.ndata['V'].size(0),0:infeaturescols]), dim = 1)
                feats=torch.hstack([graph.ndata['lk_hom'],eigenvecs])
    ...
    with torch.no_grad():
        for g_id in range(len(val_dataset)):
            ...
            with open(os.path.join(res_dir, 'results.txt'), 'a') as f:
                f.write(f'Test loss {gname}: {loss.item()}')
                f.write('\n')
        ...
        with open(os.path.join(res_dir, 'results.txt'), 'a') as f:
            f.write(f'*** Mean test loss: {test_loss/len(val_dataset)} ***')
            f.write('\n')