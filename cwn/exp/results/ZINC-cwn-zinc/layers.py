...
'''
class AbstractEmbedVEWithReduce_original(torch.nn.Module, ABC):
...
class EmbedVEWithReduce_original(AbstractEmbedVEWithReduce):
...
class AbstractEmbedVEWithReduce(torch.nn.Module, ABC):
    ...
    def __init__(self,
        ...
        self.v_embed_layer = v_embed_layer
        self.e_embed_layer = e_embed_layer
        if isinstance(v_embed_layer, torch.nn.Embedding):
            v_emb_dim = v_embed_layer.weight.size(1)
        elif isinstance(v_embed_layer, AtomEncoder):
            v_emb_dim = v_embed_layer.atom_embedding_list[0].weight.size(1)
        else:
            raise NotImplementedError
        self.v_lap_linear = Sequential(
                    Linear(10, v_emb_dim),
                    #BN(v_emb_dim),
                    torch.nn.ReLU(),
                    Linear(v_emb_dim, v_emb_dim),
                    #BN(v_emb_dim),
                    torch.nn.ReLU()
                )
        if isinstance(e_embed_layer, torch.nn.Embedding):
            e_emb_dim = e_embed_layer.weight.size(1)
        elif isinstance(e_embed_layer, BondEncoder):
            e_emb_dim = e_embed_layer.bond_embedding_list[0].weight.size(1)
        else:
            raise NotImplementedError
        if e_embed_layer is not None:
            self.e_lap_linear = Sequential(
                    Linear(10, e_emb_dim),
                    #BN(e_emb_dim),
                    torch.nn.ReLU(),
                    Linear(e_emb_dim, e_emb_dim),
                    #BN(e_emb_dim),
                    torch.nn.ReLU()
                )
        self.init_reduce = init_reduce
    ...
    def forward(self, *cochain_params: CochainMessagePassingParams):
        ...
        vx = self.v_embed_layer(self._prepare_v_inputs(v_params))
        if v_params.x.size(1) > 20:
            #vx += self.v_lap_linear(v_params.x[:, -20:].to(vx.dtype))
            vx += self.v_lap_linear(v_params.x[:, -20:-10].to(vx.dtype))
        out = [vx]
        ...
        if e_params.x is not None:
            ...
            # The output of this should be the same size as the vertex features.
            assert ex.size(1) == vx.size(1)
        if e_params.x.size(1) > 20:
            #ex += self.e_lap_linear(e_params.x[:, -20:].to(ex.dtype))
            ex += self.e_lap_linear(e_params.x[:, -20:-10].to(ex.dtype))
    ...
    def reset_parameters(self):
        reset(self.v_embed_layer)
        reset(self.e_embed_layer)
        self.v_lap_linear.reset_parameters()
        self.e_lap_linear.reset_parameters()
...
class EmbedVEWithReduce(AbstractEmbedVEWithReduce):
    ...
    def _prepare_v_inputs(self, v_params):
        assert v_params.x is not None
        assert v_params.x.dim() == 2
        if v_params.x.size(1) == 1:
            # The embedding layer expects integers so we convert the tensor to int.
            return v_params.x.squeeze(1).to(dtype=torch.long)
        else:
            return (v_params.x[:, 0:1]).squeeze(1).to(dtype=torch.long)
    
    def _prepare_e_inputs(self, e_params):
        assert self.e_embed_layer is not None
        assert e_params.x.dim() == 2
        if e_params.x.size(1) == 1:
            # The embedding layer expects integers so we convert the tensor to int.
            return e_params.x.squeeze(1).to(dtype=torch.long)
        else:
            return (e_params.x[:, 0:1]).squeeze(1).to(dtype=torch.long)
...
class OGBEmbedVEWithReduce(AbstractEmbedVEWithReduce):
    ...
    def _prepare_v_inputs(self, v_params):
        ...
        if v_params.x.size(1) <= 20:
            ...
            return (v_params.x[:, :-20]).to(dtype=torch.long)
    ...
    def _prepare_e_inputs(self, e_params):
        ...
        if e_params.x.size(1) <= 20:
            ...
            return (e_params.x[:, :-20]).to(dtype=torch.long)
