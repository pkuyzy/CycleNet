...
Our code follows from the implementation of https://github.com/twitter-research/cwn, which is the repository for the papers
...
As we have mentioned in the paper, since cell complexes have filled the cycles with cells, its cycle space may not contain any cycle. Therefore, we can either encode the original Hodge Laplacian, or encode the eigenvalues of the Hodge Laplacian, which are both basis invariant. You can directly run the code on ZINC to obtain our result. 

The original ReadMe file is available below:
