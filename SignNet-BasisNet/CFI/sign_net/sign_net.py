...
class SignNet(nn.Module):
    ...
    def reset_parameters(self):
        ...
        for layer in self.rho.children():
            if hasattr(layer, 'reset_parameters'):
                layer.reset_parameters()
        if not self.ignore_eigval:
...
class SignNetGNN(nn.Module):
    def __init__(self, node_feat, edge_feat, n_hid, n_out, nl_signnet, nl_gnn, nl_rho=4, ignore_eigval=False, gnn_type='GINEConv', pooling = 'mean'):
        super().__init__()
