...
# 2-IGN: multi 2_to_2 layers, followed by a 2_to_1 tensor, check the output layer number 1e-3

class IGN2to1(nn.Module):
    ''' batch x d x n x n -> batch x d x n
    '''

    def __init__(self, in_channels, hidden_channels, out_channels, num_layers=1, device='cuda', use_bn=False):
        ...
        # self.equi_layers.append(layer_2_to_2(in_channels, hidden_channels, device=device))
        # if self.use_bn: self.bns.append(nn.BatchNorm2d(hidden_channels))
        # self.equi_layers.append(layer_2_to_2(in_channels, 10, device=device))
        # if self.use_bn: self.bns.append(nn.BatchNorm2d(10))
        # for _ in range(2):
        #    self.equi_layers.append(layer_2_to_2(10, 10, device=device))
        #    if self.use_bn: self.bns.append(nn.BatchNorm2d(10))

        '''
        self.equi_layers.append(layer_2_to_2(in_channels, 16, device=device))
        if self.use_bn: self.bns.append(nn.BatchNorm2d(16))
        self.equi_layers.append(layer_2_to_2(16, 16, device=device))
        if self.use_bn: self.bns.append(nn.BatchNorm2d(16))
        self.equi_layers.append(layer_2_to_1(16, hidden_channels, device=device))
        ...
        for _ in range(num_layers - 2):
            self.equi_layers.append(layer_2_to_2(hidden_channels, hidden_channels, device=device))
            if self.use_bn: self.bns.append(nn.BatchNorm2d(hidden_channels))
        self.equi_layers.append(layer_2_to_1(hidden_channels, 1024, device=device))
        if self.use_bn: self.bns.append(nn.BatchNorm1d(1024))#self.bn = nn.BatchNorm1d(1024)
        self.fc1 = nn.Linear(1024, 512)
        #if self.use_bn: self.bn1 = nn.BatchNorm1d(512)
        self.fc3 = nn.Linear(512, 256)
        #if self.use_bn: self.bn3 = nn.BatchNorm1d(256)
