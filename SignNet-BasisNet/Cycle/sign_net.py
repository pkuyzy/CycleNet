    ...
    * Assume no edge feature for now.
    ...
    def __init__(self, n_in, n_out, n_layer, gnn_type='MaskedGINConv'):
        ...
        self.convs = nn.ModuleList(
            [getattr(masked_layers, gnn_type)(n_in if i == 0 else n_out, n_out, bias=True, nhid=n_out) for i in
             range(n_layer)])
...
class SignNet(nn.Module):
    ...
    def forward(self, data):
        ...
        # phi
        x = self.phi(x, data.edge_index, data.edge_attr, mask_full) + self.phi(-x, data.edge_index, data.edge_attr,
                                                                               mask_full)
...
class SignNetGNN(nn.Module):
    def __init__(self, node_feat, edge_feat, n_hid, n_out, nl_signnet, nl_gnn, nl_rho=4, ignore_eigval=False,
                 gnn_type='GINEConv', pooling='mean'):
        super().__init__()
