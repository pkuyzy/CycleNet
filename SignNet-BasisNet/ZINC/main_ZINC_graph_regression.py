...
def train_val_pipeline(MODEL_NAME, dataset, params, net_params, dirs):
    ...
    if "use_hodge" in net_params and net_params["use_hodge"] in ["basis", "PEOI"]:
        dataset._add_hodge_encodings()

    if net_params['pe_init'] == 'lap_pe':
        tt = time.time()
        print("[!] -LapPE: Initializing graph positional encoding with Laplacian PE.")
        #dataset._add_cycle_encodings(net_params['pos_enc_dim'])
    ...
    if net_params["lap_method"] != "basis_inv":
        ...
        train_loader = DataLoader(trainset, num_workers=4, batch_size=params['batch_size'], shuffle=True,
                                  collate_fn=dataset.collate_basis)
...
def main():
    ...
    parser.add_argument('--config', help="Please give a config.json file with training/model/data/param details", default = './configs/pna/PNA_ZINC_LapPE_signinv_GIN_mask.json')#PNA_ZINC_NoPE_smallbatch.json')#gin/GIN_ZINC_NoPE_smallbatch.json')
    ...
    parser.add_argument('--use_hodge', default = "basis", help="whether use the cycle information or not, can be basis, PEOI or None")
    ...
    if args.use_hodge is not None:
        net_params["use_hodge"] = args.use_hodge

    print('BATCH NORM', args.batch_norm)

    time_str = time.strftime('%Hh%Mm%Ss_on_%b_%d_%Y')
    save_models_dir = out_dir + 'checkpoints/' + MODEL_NAME + "_" + DATASET_NAME + "_GPU" + str(
        config['gpu']['id']) + "_" + time_str
    if not os.path.exists(save_models_dir):
        os.makedirs(save_models_dir)
    copy("main_ZINC_graph_regression.py", save_models_dir)
    copy("./train/train_ZINC_graph_regression.py", save_models_dir)
    copy("./nets/ZINC_graph_regression/gin_net.py", save_models_dir)
    copy("./nets/ZINC_graph_regression/gatedgcn_net.py", save_models_dir)
    copy("./nets/ZINC_graph_regression/pna_net.py", save_models_dir)
    copy("./layers/ign.py", save_models_dir)
    copy("./layers/deepsigns.py", save_models_dir)
    copy("./data/molecules.py", save_models_dir)
    ...
    root_log_dir = out_dir + 'logs/' + MODEL_NAME + "_" + DATASET_NAME + "_GPU" + str(config['gpu']['id']) + "_" + time_str#time.strftime('%Hh%Mm%Ss_on_%b_%d_%Y')
    root_ckpt_dir = out_dir + 'checkpoints/' + MODEL_NAME + "_" + DATASET_NAME + "_GPU" + str(config['gpu']['id']) + "_" + time_str#time.strftime('%Hh%Mm%Ss_on_%b_%d_%Y')
    write_file_name = out_dir + 'results/result_' + MODEL_NAME + "_" + DATASET_NAME + "_GPU" + str(config['gpu']['id']) + "_" + time_str#time.strftime('%Hh%Mm%Ss_on_%b_%d_%Y')
    write_config_file = out_dir + 'configs/config_' + MODEL_NAME + "_" + DATASET_NAME + "_GPU" + str(config['gpu']['id']) + "_" + time_str#time.strftime('%Hh%Mm%Ss_on_%b_%d_%Y')
    viz_dir = out_dir + 'viz/' + MODEL_NAME + "_" + DATASET_NAME + "_GPU" + str(config['gpu']['id']) + "_" + time_str#time.strftime('%Hh%Mm%Ss_on_%b_%d_%Y')
