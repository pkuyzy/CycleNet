...
def handle_lap(model, batch_pos_enc, batch_graphs, device, Projs):
    ...
    elif model.lap_method == 'basis_inv':
        batch_pos_enc = batch_pos_enc.unsqueeze(-1)
        batch_pos_enc = model.sign_inv_net(batch_graphs, batch_pos_enc, Projs)
        # k x n x 1 -> n x k
        batch_pos_enc = batch_pos_enc.squeeze(-1)
...
def train_epoch_sparse(model, optimizer, device, data_loader, epoch):
    ...
    nb_data = 0
    gpu_mem = 0
    for iter, (batch_graphs, batch_targets, batch_snorm_n, Projs, batch_info, hodge_emb) in enumerate(data_loader):
        ...
        if Projs is not None:
            for same_size_projs in Projs:
                for mult in same_size_projs:
                    same_size_projs[mult] = same_size_projs[mult].to(device)
        ...
        #try:
        #    batch_pos_enc_edge = batch_graphs.edata['pos_enc'].to(device)
        #except KeyError:
        #    batch_pos_enc_edge = None

        if model.pe_init == 'lap_pe':
            #batch_pos_enc = batch_pos_enc
            batch_pos_enc = handle_lap(model, batch_pos_enc, batch_graphs, device, Projs)

        if not hasattr(model, 'ign'):
            batch_scores, __ = model.forward(batch_graphs, batch_x, batch_pos_enc, batch_e, batch_snorm_n, hodge_emb)
        else:
            batch_info = batch_info.to(device)
            batch_scores, __ = model.forward(batch_graphs, batch_x, batch_pos_enc, batch_e, batch_snorm_n, batch_info)
        del __
...
def evaluate_network_sparse(model, device, data_loader, epoch):
    ...
    with torch.no_grad():
        for iter, (batch_graphs, batch_targets, batch_snorm_n, Projs, batch_info, hodge_emb) in enumerate(data_loader):
            ...
            if not hasattr(model, 'ign'):
                batch_scores, batch_g = model.forward(batch_graphs, batch_x, batch_pos_enc, batch_e, batch_snorm_n, hodge_emb)
            else:
                batch_info = batch_info.to(device)
                batch_scores, batch_g = model.forward(batch_graphs, batch_x, batch_pos_enc, batch_e, batch_snorm_n,
                                                 batch_info)
