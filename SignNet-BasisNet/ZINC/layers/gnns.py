...
from layers.gin_conv import GINConv as GINConv_tg
...
class GIN_tg(nn.Module):
    def __init__(self, in_channels, hidden_channels, out_channels, n_layers, use_bn=True, dropout=0.5, activation='relu'):
        ...
        self.activation = activations[activation]
        # input layer
        update_net = MLP(in_channels * 2, hidden_channels, hidden_channels, 2, use_bn=use_bn, dropout=dropout, activation=activation)
        self.layers.append(GINConv_tg(update_net))
        # hidden layers
        for i in range(n_layers - 2):
            update_net = MLP(hidden_channels + in_channels, hidden_channels, hidden_channels, 2, use_bn=use_bn, dropout=dropout, activation=activation)
            self.layers.append(GINConv_tg(update_net))
            if use_bn: self.bns.append(nn.BatchNorm1d(hidden_channels))
        # output layer
        update_net = MLP(hidden_channels + in_channels, hidden_channels, out_channels, 2, use_bn=use_bn, dropout=dropout, activation=activation)
        self.layers.append(GINConv_tg(update_net))
        if use_bn: self.bns.append(nn.BatchNorm1d(hidden_channels))
        self.dropout = nn.Dropout(p=dropout)
            ...
            if i != 0:
                ...
                if self.use_bn:
                    if x.ndim == 2:
                        ...
                        raise ValueError('invalid x dim')
            x = layer(x, torch.cat([e.view(1, -1) for e in g.edges()], dim = 0), g.edata['pos_enc'])
        return x
